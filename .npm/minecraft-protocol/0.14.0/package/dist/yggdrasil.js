...
function getSession(username, password, clientToken, refresh, cb) {
  if (refresh) {
    ...
    superagent.post(loginSrv + "/refresh").type("json").send({
    ...
    superagent.post(loginSrv + "/authenticate").type("json").send({
      "agent": {
        "name": "Minecraft",
        "version": 1
      },
...
function joinServer(username, serverId, accessToken, selectedProfile, cb) {
  superagent.post("https://sessionserver.mojang.com/session/minecraft/join").type("json").send({
    "accessToken": accessToken,
...
function validateSession(username, serverId, cb) {
  superagent.get("https://sessionserver.mojang.com/session/minecraft/hasJoined?username=" + username + "&serverId=" + serverId).end(function (resp) {
    if (resp.ok) {
      if ("id" in resp.body) {
...
//# sourceMappingURL=maps/yggdrasil.js.map