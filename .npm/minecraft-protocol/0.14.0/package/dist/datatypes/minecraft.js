...
function readUUID(buffer, offset) {
  return {
    value: [buffer.readUInt32BE(offset), buffer.readUInt32BE(offset + 4), buffer.readUInt32BE(offset + 8), buffer.readUInt32BE(offset + 12)],
    size: 16 };
}
...
function writePosition(value, buffer, offset) {
  var longVal = [];
  longVal[0] = (value.x & 67108863) << 6 | (value.y & 4095) >> 6;
  longVal[1] = (value.y & 63) << 26 | value.z & 67108863;
...
function sizeOfSlot(value) {
  if (value.blockId === -1) return 2;else if (!value.nbtData) {
    return 6;
  } else {
...
var entityMetadataTypes = {
  ...
  6: {
    type: "container", typeArgs: {
      fields: [{ name: "x", type: "int" }, { name: "y", type: "int" }, { name: "z", type: "int" }]
    }
  },
  7: {
    type: "container", typeArgs: {
      fields: [{ name: "pitch", type: "float" }, { name: "yaw", type: "float" }, { name: "roll", type: "float" }]
    }
  }
};
