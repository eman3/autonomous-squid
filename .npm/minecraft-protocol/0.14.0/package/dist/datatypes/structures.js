...
function readArray(buffer, offset, typeArgs, rootNode) {
  ...
  } else count = getField(typeArgs.count, rootNode);
...
function writeContainer(value, buffer, offset, typeArgs, rootNode) {
  ...
  rootNode["this"] = value;
  for (var index in typeArgs.fields) {
    if (!context.hasOwnProperty(typeArgs.fields[index].name) && typeArgs.fields[index].type != "count" && (typeArgs.fields[index].type != "condition" || evalCondition(typeArgs.fields[index].typeArgs, rootNode))) {
