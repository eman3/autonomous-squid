...
function createServer(options) {
  options = options || {};
  var port = options.port != null ? options.port : options['server-port'] != null ? options['server-port'] : 25565;
  ...
  server.on('connection', function (client) {
    client.once([states.HANDSHAKING, 0], onHandshake);
    client.once([states.LOGIN, 0], onLogin);
    client.once([states.STATUS, 0], onPing);
    client.on('end', onEnd);
    ...
    function onPing(packet) {
      ...
      client.once([states.STATUS, 1], function (packet) {
        client.write(1, { time: packet.time });
        client.end();
      });
      client.write(0, { response: JSON.stringify(response) });
    }
    ...
    function onLogin(packet) {
      ...
      if (needToVerify) {
        ...
        client.once([states.LOGIN, 1], onEncryptionKeyResponse);
        client.write(1, {
          serverId: serverId,
    ...
    function loginClient() {
      ...
      client.write(2, { uuid: client.uuid, username: client.username });
...
function createClient(options) {
  ...
  if (keepAlive) client.on([states.PLAY, 0], onKeepAlive);
  client.once([states.LOGIN, 1], onEncryptionKeyRequest);
  client.once([states.LOGIN, 2], onLogin);
  client.once('compress', onCompressionRequest);
  client.on('set_compression', onCompressionRequest);
  if (haveCredentials) {
    // make a request to get the case-correct username before connecting.
    var cb = function cb(err, session) {
      if (err) {
    ...
    if (accessToken != null) getSession(options.username, accessToken, clientToken, true, cb);else getSession(options.username, options.password, clientToken, false, cb);
  } else {
...
function mcHexDigest(hash) {
  ...
  function performTwosCompliment(buffer) {
    ...
    for (i = buffer.length - 1; i >= 0; --i) {
      ...
      if (carry) {
        carry = newByte === 255;
        buffer.writeUInt8(newByte + 1 & 255, i);
      } else {
